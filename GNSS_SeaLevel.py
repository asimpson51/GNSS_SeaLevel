...
def fit_tide_gauge(filename):
    ...
    velocity, uncertainty = fit_timeseries(df.iloc[:, 0], df.iloc[:, 1])
    sealevel_df = pd.DataFrame({
        'Site': site,
        'Sea-Level Rise': velocity
    })
    return sealevel_df
...
def fit_all_velocities(folder, pattern, tname, data_type, ename=None, nname=None, uname=None, lat=None, lon=None, elev=None):
    results = []
    ...
    for file_path in file_paths:
        ...
        if data_type == 'tide':
            velocities = fit_tide_gauge(file_path, tname, ename, nname, uname)
        elif data_type == 'GNSS' or data_type == 'gnss':
            if ename is None or nname is None or uname is None or lat is None or lon is None or elev is None:
                raise ValueError("For GNSS data, ename, nname, uname, lat, lon, and elev must be provided.")
